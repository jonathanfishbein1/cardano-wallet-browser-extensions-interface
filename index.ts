...
const getUtxos = async wallet => {
    const rawUtxos = ('Typhon Wallet' === wallet.name) ?
        []
        :
...
const delegateTo = async (wallet, poolId, protocolParameters, account) => {
    if ('Typhon Wallet' === wallet.name) {
        ...
        utxos.map((utxo: CSL.TransactionUnspentOutput) => inputBuilder.add_input(utxo.output().address(), utxo.input(), utxo.output().amount()))
